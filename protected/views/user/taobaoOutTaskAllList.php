    ...
    <div class="dtrw"><!--大厅任务-->
        <div class="dtrwCen">
            <div class="dtrwCen_t">
                <div class="rwso">
                    <form action="<?php echo $this->createUrl('user/taobaoOutTaskAllList');?>" method="post">
            ...
            <li class="taskItem">
                ...
                <div class="allRw_pro">
                    ...
                    <?php
                        if($item->status==0){
                    ?>
                        <a class="qcrw"><?php if($item->endtime >= time() or $item->endtime==0) { ?>等待接手<?php }else{ ?>已过期<?php } ?></a>
                    ...
                    <?php
                        ...
                        if($item->status==4){
                    ?>
                         <?php $hourarr=array("0" => 0,"1" => 24,"2" => 48,"3" => 72,"4" => 96,"5" => 120,"6" => 144,"7" => 168,);
						$needhour=$hourarr[$item->ddlOKDay];
						$querenshouhuotime=$item->taskforthTime+$needhour*3600;
						?>
                        <a href="javascript:;" class="qcrw" lang="<?php echo $item->id;?>" style="width: auto; padding:0 8px; cursor: pointer;">等待威客收货好评</a>
						<div class="clear"></div>
                        <span style="width: auto; position: relative; top:-30px; left:805px;" class="settime" action="querenshouhuotime" lang="<?php echo $item->taskerid;?>" alt="<?php echo $item->id;?>" endTime="<?php echo date("Y-m-d H:i:s",$querenshouhuotime);?>"></span>
                    
                    <?php
                        }
                        if($item->status==5 && $item->taskCompleteStatus==0 && $item->complian_status==0){
                    ?>
                        <a href="javascript:;" class="qcrw sellerLastCertain" lang="<?php echo $item->id;?>" style="width: auto; padding:0 8px; cursor: pointer; margin-top:-9px;">任务完成，等待您确认</a><br />
                ...
                <div class="takerInfo"><!--taskFunMan start-->
				    ...
				    <span style="color: red;"><?php
                        if($item->orderNumber)
                            echo "订单编号：".$item->orderNumber."<font style='color:#000'>(核实是否付款)</font>";
                    ?></span>
                </div><!--taskFunMan end-->
                <?php
                    }
                ...
                <div class="taskFunMan"><!--taskFunMan start-->
                    ...
                    <?php
                        ...
                        if($item->status==0){
                    ?>
					<font><a href="javascript:;" class="deleteTask" lang="<?php echo $item->taskerid;?>" alt="<?php echo $item->id;?>">删除</a></font>
                    <font><a href="javascript:;" class="stopTask" lang="<?php echo $item->taskerid;?>" alt="<?php echo $item->id;?>">暂停</a></font>
                    <?php }?>
                    <?php
                        //任务没有完成的状态下，商家都可以进行追加豆豆
                        if($item->taskCompleteStatus==0 && $item->complian_status<>1 && $item->status<3){
                    ?>
                    <font><a href="javascript:;" class="addMinLi" lang="<?php echo $item->taskerid;?>" alt="<?php echo $item->id;?>">追加豆豆</a></font>
                    <?php }?>
                    <?php
        ...
        function updateEndTime()
        {
            ...
            $(".settime").each(function(i){
                ...
                if(lag > 0)
                {
                    ...
                    {
                        actionName="自动发货";
                    }else if($(this).attr("action")=="querenshouhuotime"){
						actionName="确认收货";
					}
                    else
                    {
    ...
    <script>
        $(function(){
            ...
            $(".deleteTask").click(function(){
                var taskerid=$(this).attr("lang");
                var id=$(this).attr("alt");
                layer.confirm('您确定要删除任务吗？', {
            		btn: ['确定删除','不删除'] //按钮
            	},function(){
            	   $.ajax({
            			type:"POST",
            			url:"<?php echo $this->createUrl('site/deleteTask');?>",
            			data:{"taskerid":taskerid,"id":id},
            			success:function(msg)
            			{
                            if(msg=="SUCCESS")
                            {
           			            layer.confirm('任务已成功删除', {
                            		btn: ['知道了'] //按钮
                            	},function(){
                            	   location.reload();//重新刷新当前页面
                            	});
                            ...
                            {
                                layer.confirm('任务已被删除或进行中无法执行操作', {
                            		btn: ['知道了'] //按钮
                            	},function(){
                            	   location.reload();//重新刷新当前页面
                            	});
            ...
            $(".addMinLi").click(function(){
                ...
                layer.confirm('<span style="color:red; line-height:25px;">您剩余豆豆'+$(".MinLinOwn").html()+'个</span><br/>'+'追加豆豆数：<input type="text" class="text1 addMinLiNum" style="width:50px; text-align:center; margin:0;" />&nbsp;个', {
            		btn: ['确定追加','取消'] //按钮
            	},function(){
                   ...
                   if(isNaN(parseFloat($(".addMinLiNum").val())))//豆豆数量必须为数字
                   {
                        layer.tips('<span>必须为数字<span>', '.addMinLiNum');
                        exit;
                   }
                   if(parseFloat($(".addMinLiNum").val())<0)//豆豆数量必须大于0
                   {
                        layer.tips('<span>必须大于0<span>', '.addMinLiNum');
                        exit;
                   }
                   
                   if(parseFloat($(".addMinLiNum").val())>parseFloat($(".MinLinOwn").html()))//豆豆剩余数量不足
                   {
                        layer.tips('<span>豆豆数量不足，您剩余豆豆'+$(".MinLinOwn").html()+'个<span>', '.addMinLiNum');
                        exit;
                   }
                   ...
                   $.ajax({
            			...
            			success:function(msg)
            			{
                            ...
                            {
                                layer.confirm('豆豆数量不足', {
                            		btn: ['知道了'] //按钮
                            	});
                            }
                            else
                            {
                                layer.confirm('豆豆追加失败，请联系我们的客服人员', {
                            		btn: ['知道了'] //按钮
                            	});
                            }
            			}
            ...
            $(".sellerLastCertain").click(function(){
                ...
                layer.confirm('<span style="font-size:12px;"><p style="font-size:14px; font-weight:bold; text-align:center; color:red;">请注意！您即将要确认付款给对方了</p>请确认您已经在淘宝收到接手的货款与好评，并且完成了您的任务，确定后，任务担保金和豆豆将到达对方平台帐户！<p style="color:red; font-weight:bold; font-size:14px; text-align:center;">如果对方未付款，请勿确认，请直接发布申诉。</p></span>', {
            		btn: ['确认','取消'] //按钮
            	},function(){
