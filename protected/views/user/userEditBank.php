        ...
        <div class="setInside">
            <table>
                <input type="hidden" name="setPhon" value="Done" />
				 <input type="hidden" name="id" value="<?php if(isset($_GET['bankid'])) echo $_GET['bankid'];?>"/>
                <tr>
                    <td>提现银行：</td>
                    <td>
                        <select name="bankCatalog" id="bankCatalog">
                			<option value="2" <?php if($bankInfo->bankCatalog == 2) { ?>selected<?php } ?>>中国工商银行</option>
                			<option value="4" <?php if($bankInfo->bankCatalog == 4) { ?>selected<?php } ?>>中国建设银行</option>
                			<option value="7" <?php if($bankInfo->bankCatalog == 7) { ?>selected<?php } ?>>中国农业银行</option>
                        </select>
                    </td>
                    <td></td>
                </tr>
                ...
                <tr>
                    <td>银行卡号：</td>
                    <td>
                        <input name="bankAccount" placeholder="银行卡号" class="text2 bankAccount" style=" text-indent: 10px;" type="text" value="<?php echo $bankInfo->bankAccount;?>">
                    </td>
            ...
            $.ajax({
    			...
    			success:function(msg)
    			{
    				if(msg=="SUCCESS")//手机验证码正确
                    {
                        ...
                        $.ajax({
                			type:"POST",
                			url:"<?php echo $this->createUrl('user/userEditBankCertain');?>",
                			...
                			success:function(msg)
                			{
                                ...
                                {
                                    //询问框
                                	layer.confirm('<span style="color:red;">银行卡修改失败</span>您可以联系我们的客服人员', {
                               		   btn: ['知道了'] //按钮
                                	});
                                }
                			}
